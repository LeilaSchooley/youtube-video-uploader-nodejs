...
async function processQueueItem(item: QueueItem): Promise<void> {
  ...
  try {
    ...
    // Simplified: Upload all videos immediately with publishAt dates from CSV
    // YouTube will handle the publishing schedule automatically
    console.log(
      `[WORKER] [${new Date().toISOString()}] Job ${item.id}: Processing all ${
        csvData.length
      } videos immediately. Videos will be scheduled using publishAt dates from CSV.`
    );
    ...
    // Process all videos immediately - no interval limits, no scheduling delays
    for (let i = 0; i < csvData.length; i++) {
      ...
      // Determine publish date from CSV scheduleTime field
      // If scheduleTime is provided, use it. Otherwise, upload immediately without scheduling.
      ...
      if (scheduleTime) {
        ...
        console.log(
          `[WORKER] [${new Date().toISOString()}] Video ${
            i + 1
          }: Will be scheduled for ${publishDate.toISOString()}`
        );
      }

      // Upload all videos immediately with publishAt dates
      // If there's a publishDate, upload as private with publishAt (YouTube will publish automatically)
