/**
 * List all uploaded files (by channel). Used by scripts or future "All Files" UI; not used by dashboard.
 */
...
function scanDirectory(
  dir: string,
  relativeTo: string = "",
): Array<{ name: string; path: string; relativePath: string; size: number }> {
  const results: Array<{
    name: string;
    path: string;
    relativePath: string;
...
export async function GET(request: NextRequest) {
  try {
    ...
    const safeTargetUserId = targetUserId
      ? targetUserId.replace(/[^a-zA-Z0-9._-]/g, "_")
      : null;
    ...
    const userJobs = queue.filter((item) => {
      if (requestedChannel) {
        ...
        const itemSafeUserId = item.userId
          ? item.userId.replace(/[^a-zA-Z0-9._-]/g, "_")
          : null;
    ...
    const thumbnailCount = allFiles.filter(
      (f) => f.type === "thumbnail",
    ).length;
    ...
    const filesByJob = allFiles.reduce(
      (acc, file) => {
        if (!acc[file.jobId]) {
          acc[file.jobId] = {
            jobId: file.jobId,
    ...
    console.log(
      `[LIST-ALL-FILES] Session: ${sessionId.substring(0, 10)}..., UserId: ${userId || "N/A"}`,
    );
    console.log(`[LIST-ALL-FILES] Safe UserId: ${safeUserId || "N/A"}`);
    console.log(
      `[LIST-ALL-FILES] Queue jobs: ${userJobs.length}, Files found: ${allFiles.length}, Orphans: ${orphanCount}`,
    );
    console.log(
      `[LIST-ALL-FILES] Uploads dir: ${uploadsDir}, exists: ${fs.existsSync(uploadsDir)}`,
    );
    if (userUploadDir) {
      console.log(
        `[LIST-ALL-FILES] User upload dir: ${userUploadDir}, exists: ${fs.existsSync(userUploadDir)}`,
      );
    }
    console.log(
      `[LIST-ALL-FILES] Session upload dir: ${sessionUploadDir}, exists: ${fs.existsSync(sessionUploadDir)}`,
    );
