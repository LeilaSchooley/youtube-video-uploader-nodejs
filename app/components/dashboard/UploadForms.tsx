...
export default function UploadForms({
  ...
  useEffect(() => {
    if (typeof window !== "undefined") {
      ...
      if (savedVideosPerDay) {
        ...
        const videosPerDayInput = document.getElementById(
          "videosPerDay",
        ) as HTMLInputElement;
  ...
  useEffect(() => {
    ...
    fetch(`/api/dropbox-sheet-names?filePath=${encodeURIComponent(path)}`)
      .then((res) => res.json())
      .then((data) => {
        if (cancelled) return;
  ...
  useEffect(() => {
    // If we just returned from Dropbox OAuth callback, skip cache and force fresh check
    const fromDropboxCallback =
      typeof window !== "undefined" &&
      searchParams.get("dropbox_connected") === "1";
    if (fromDropboxCallback && typeof window !== "undefined") {
      localStorage.removeItem("hasDropboxAuth");
      setHasDropboxAuth(null);
    }

    // First, load cached auth state from localStorage immediately (unless we just came from Dropbox callback)
    ...
    const checkDropboxAuth = async () => {
      try {
        ...
        // Remove dropbox_connected from URL after handling
        if (fromDropboxCallback && typeof window !== "undefined") {
          ...
          <div className="space-y-4">
                    ...
                    <div className="flex items-start gap-2 mb-3">
                      ...
                      <div className="flex-1">
                        ...
                        <p className="text-sm text-blue-800 dark:text-blue-200 mb-3">
                          Upload all videos from a Dropbox folder. Supports
                          recursive folder scanning and post-upload actions.
                        </p>
                            ...
                            {hasDropboxAuth === null ? (
                              ...
                              <>
                                <p className="text-sm text-blue-900 dark:text-blue-100 mb-2">
                                  Connect your Dropbox account to use Dropbox
                                  folders
                                </p>
                                <button
                                  type="button"
                        ...
                        {hasDropboxAuth !== true && (
                          <div className="mt-2 p-3 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
                            <p className="text-sm text-blue-800 dark:text-blue-200">
                              {hasDropboxAuth === null
                                ? "Checking Dropbox connection..."
                                : "Connect your Dropbox account above to enable file selection"}
                            </p>
                          </div>
                        )}
                      </div>
                  ...
                  <button
                    ...
                    onClick={async () => {
                      ...
                      try {
                        ...
                        const videosPerDayNum = videosPerDay
                          ? parseInt(videosPerDay, 10)
                          : undefined;
                        if (
                          videosPerDayNum !== undefined &&
                          (isNaN(videosPerDayNum) || videosPerDayNum < 0)
                        ) {
                          setShowToast({
            ...
            <div className="p-4 bg-blue-50 dark:bg-blue-900/20 border border-blue-200 dark:border-blue-700 rounded-lg">
              ...
              <div>
                ...
                />
                <p className="text-xs text-gray-500 dark:text-gray-400 mt-1">
                  Leave 0 or empty to upload all immediately. If set, videos
                  will upload X per day starting today.
                </p>
              </div>
              <div className="mt-3 p-3 bg-blue-100 dark:bg-blue-900/30 rounded text-xs text-blue-800 dark:text-blue-200">
                ...
                the first 5 videos will upload today, videos 6-10 tomorrow, and
                so on. Videos are uploaded immediately but scheduled to publish
                on their assigned dates.
                <br />
                <strong>Note:</strong> This setting applies to all upload
                methods (Sheets, Dropbox, CSV). If a video has a{" "}
                ...
                date in your sheet/CSV, that date will be used instead.
              </div>
            </div>
          </div>
        )}
